...
Training:
python ray_train_multirun.py --config-name=train_diffusion_unet_scara_workspace --seeds=42,43,44 --monitor_key=test_mean_score -- logging.mode=online task.dataset_path=/pad/naar/greenhouse_demos
"""
import os
import ray
import click
...
def main(config_name, config_dir, seeds, monitor_key, ray_address, 
    ...
    if config_dir is None:
        config_path_abs = pathlib.Path(__file__).parent.joinpath(
            'greenhouse_diffusion_policy','config')
    ...
    with hydra.initialize(
        ...
        monitor_command_args = [
            ...
            '--use_wandb',
            '--project', 'greenhouse_diffusion_policy_metrics',
