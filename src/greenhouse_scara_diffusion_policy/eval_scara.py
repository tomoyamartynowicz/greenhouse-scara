...
from greenhouse_scara_common.data import image_specs
from greenhouse_scara_common.live import MultiCameraSource, observation_from_history, preview_image, DEFAULT_SERIALS
...
from greenhouse_diffusion_policy.policy.base_image_policy import BaseImagePolicy
from greenhouse_diffusion_policy.workspace.base_workspace import BaseWorkspace
...
def get_scara_shape_meta(cfg):
    if list(cfg.task.shape_meta.obs.qpos.shape) != [4] or list(cfg.task.shape_meta.action.shape) != [4]:
        raise ValueError('Expected qpos and actions of shape [4]')
    return image_specs(cfg.task.shape_meta)


def make_policy_observation(history, specs, device):
    return observation_from_history(history, specs, device)


def capture_history(
    camera: MultiCameraSource,
    robot: TCSReadClient,
    n_obs_steps: int,
    ...
    for _ in range(n_obs_steps):
        sample = camera.read()
        history.append((sample, robot.get_joint_vector()))
    return history
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--serial", default=None, help="Serial override for a single physical camera")
    parser.add_argument("--camera", action="append", nargs=2, metavar=("NAME", "SERIAL"))
...
def main() -> None:
    ...
    specs = get_scara_shape_meta(cfg)
    serials = dict(DEFAULT_SERIALS)
    if args.camera: serials.update(dict(args.camera))
    if args.serial:
        names = {spec.get('camera', key) for key, spec in specs.items()}
        if len(names) != 1: raise ValueError('Use --camera NAME SERIAL for multiple cameras')
        serials[next(iter(names))] = args.serial
    n_obs_steps = int(cfg.n_obs_steps)
    camera = MultiCameraSource(specs, serials, size=(args.camera_width, args.camera_height), fps=args.camera_fps)
    ...
    try:
        ...
        print(
            f"Mode={mode}, streams={list(specs)}, obs_steps={n_obs_steps}, "
        ...
        for chunk_index in range(args.chunks):
            obs = make_policy_observation(history, specs, device)
            ...
            if args.show_camera:
                cv2.imshow("SCARA Diffusion Policy", preview_image(history[-1][0])[..., ::-1])
